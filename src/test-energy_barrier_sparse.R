...
# Allstates.tsv
#######################################
Allstates <- as.matrix(read.table("output_sparse/Allstates.tsv", header=FALSE))
...
# Freq.tsv
#######################################
Freq <- read.table("output_sparse/Freq.tsv", header=FALSE)
# Size: States
...
# P_emp.tsv
#######################################
P_emp <- as.matrix(read.table("output_sparse/P_emp.tsv", header=FALSE))
# Size: States
...
# P_est.tsv
#######################################
P_est <- as.matrix(read.table("output_sparse/P_est.tsv", header=FALSE))
# Size: States
...
# h.tsv
#######################################
h <- as.matrix(read.table("output_sparse/h.tsv", header=FALSE))
# Size: Variables
...
expect_true(cor(h, h_elat) > 0.3)
# Correlation with Ground Truth h
h_true <- c(-0.3, 0.1, 0.4, -0.2, 0.3, 0.4, -0.1)
expect_true(cor(h, h_true) > 0.3)

#######################################
# J.tsv
#######################################
J <- as.matrix(read.table("output_sparse/J.tsv", header=FALSE))
...
expect_true(cor(as.vector(J), as.vector(J_true)) > 0.6)
...
0.3073, 0.2760,
-0.0299)
expect_true(cor(J[lower.tri(J)], J_elat) > 0.6)

#######################################
# E.tsv
#######################################
E <- as.matrix(read.table("output_sparse/E.tsv", header=FALSE))
# Size: States
