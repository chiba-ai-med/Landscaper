...
# Freq.tsv
#######################################
Freq <- read.table("output_cov/Freq.tsv", header=FALSE)
# Size: States
...
# P_emp.tsv
#######################################
P_emp <- as.matrix(read.table("output_cov/P_emp.tsv", header=FALSE))
# Size: States
...
# P_est.tsv
#######################################
P_est <- as.matrix(read.table("output_cov/P_est.tsv", header=FALSE))
# Size: States
...
# h.tsv
#######################################
h <- as.matrix(read.table("output_cov/h.tsv", header=FALSE))
# Size: Variables
...
expect_true(cor(h, h_elat) > 0.6)
# Correlation with Ground Truth h
h_true <- c(-0.3, 0.1, 0.4, -0.2, 0.3, 0.4, -0.1)
expect_true(cor(h, h_true) > 0.6)

#######################################
# J.tsv
#######################################
J <- as.matrix(read.table("output_cov/J.tsv", header=FALSE))
...
# E.tsv
#######################################
E <- as.matrix(read.table("output_cov/E.tsv", header=FALSE))
# Size: States
